...
while [ $n -lt ${nlines} ]; do
    ...
    if [ $1 = ${SCERN} ]; then
     ...
     cat condor_template_FNAL.cfg  | sed "s?4mu?$4?g" | sed "s?submit_HZZ4LeptonsAnalysis_FNAL?submit_ReferenceAnalysis_sig_${samplename}_$4?g" | sed "s?RunReferenceAnalysis?RunReference$4_signal?g" | sed "s?sig_input_h150.txt?SigCards$4$3/sig_input_${n}.txt?g" | sed "s?mail?`whoami`?g" | sed "s?TARfile?${tarname}.tgz?g"  > jobs$4/condor_ReferenceAnalysis_sig_${samplename}_$4.cfg
