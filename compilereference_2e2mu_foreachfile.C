...
int main (int argc, char ** argv){
  ...
  for(int i=0;i<signlines;i++){
    ...
    if (runonsig==true){
      ...
      sprintf(nome,"histos2e2mu/output_%s.root",sigsamples[i].c_str());
      make3.Loop(nome);
	...
	//sprintf(nome,"histos2e2mu/output_%s_H%d.root",datasamples[i].c_str(),int(mH));
	//char *path=NULL;
	//size_t size=300;
...
// 	//sprintf(nome,"histos2e2mu/output_%s_H%d.root",bkgsamples[i].c_str(),int(mH));
        sprintf(nome,"output.root");

// 	sprintf(nome,"histos2e2mu_DoubleMu_Run2012B_PromptReco_Jun22_190456-196509_noLowPU/output_%s_H%d.root",alpha.Data(),int(mH));
 	make3.Loop(nome);
